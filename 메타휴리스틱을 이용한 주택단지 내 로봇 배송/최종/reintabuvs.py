...
class Config:
    ...
    # ----------------------------
    INSERTION_SAMPLES = 15  # (i,j) 후보 15개 샘플링 평가
    ...
    ANIMATE_SEED_INDEX = 0

    OUT_JSON = "compare_greedy_vs_tabu_runs.json"
...
# =============================================================================
# 8) Tabu Search (reassign + best insertion)
# =============================================================================
...
# =============================================================================
# 9) Assignment (Greedy-only uses append; Tabu-policy uses best insertion)
# =============================================================================
def assign_order_greedy(new_items, robots, log_stack, current_time, timing_stats,
                        use_tabu=False, tabu_list=None, rng=None):
    for item in new_items:
        ...
        if use_tabu and rng is not None:
            ...
            best_robot.route.append(make_pickup_task(item))
            best_robot.route.append(make_delivery_task(item))
        ...
        if use_tabu and tabu_list is not None and rng is not None:
...
# =============================================================================
# 12) Headless simulation (for figure2 + JSON)
# =============================================================================
def p95(arr):
    if not arr:
        return 0.0
...
def summarize(policy_name, robots, items, timing):
    ...
    return {
        ...
        "p95_event_compute_ms": round(p95(ev), 3) if ev else 0.0,
    }, thr_list

def run_simulation_headless(events, policy_name, seed, use_tabu: bool):
    rng_policy = RNG(seed + (999 if use_tabu else 0))
    ...
    tabu_list = deque(maxlen=Config.TABU_SIZE) if use_tabu else None
    ...
    for t in range(1, Config.RUN_DURATION_MIN + 1):
        while idx < len(events) and events[idx]["t"] == t:
            ...
            assign_order_greedy(
                new_items, robots, log, t, timing,
                use_tabu=use_tabu, tabu_list=tabu_list, rng=rng_policy
            )
    ...
    summary, thr = summarize(policy_name, robots, items, timing)
    return summary, thr


# =============================================================================
# 13) Figure1 animation (Tabu only) + live KPI x2
# =============================================================================
def run_tabu_animation(events, seed):
    ...
    def update(frame):
        ...
        while idx < len(events) and events[idx]["t"] == t:
            ...
            assign_order_greedy(
                new_items, robots, persistent_event_log, t, timing,
                use_tabu=True, tabu_list=tabu_list, rng=rng_policy
            )
...
# =============================================================================
# 14) Figure2: multi-seed comparison + paired diff
# =============================================================================
def plot_comparison_figure2(results_by_seed):
    import numpy as np

    seeds = [r["seed"] for r in results_by_seed]

    def get(metric, key):
        return [r[key][metric] for r in results_by_seed]

    def mean_std(a):
        return float(np.mean(a)), float(np.std(a, ddof=0))

    greedy_key = "greedy_summary"
    tabu_key = "tabu_summary"

    fig2 = plt.figure(figsize=(16, 11))
    fig2.suptitle("Greedy-only vs Tabu(best-insertion) (multi-seed) + Paired Differences", fontsize=14)

    ax1 = fig2.add_subplot(3, 3, 1)
    g = get("on_time_rate_pct", greedy_key)
    t = get("on_time_rate_pct", tabu_key)
    g_m, g_s = mean_std(g)
    t_m, t_s = mean_std(t)
    ax1.bar(["Greedy", "Tabu"], [g_m, t_m], yerr=[g_s, t_s], capsize=6)
    ax1.set_title("On-time rate (%)\n(mean ± std)")
    ax1.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax2 = fig2.add_subplot(3, 3, 2)
    g_p95 = get("p95_lead_time_min", greedy_key)
    t_p95 = get("p95_lead_time_min", tabu_key)
    g_max = get("max_lead_time_min", greedy_key)
    t_max = get("max_lead_time_min", tabu_key)

    g_p95_m, g_p95_s = mean_std(g_p95)
    t_p95_m, t_p95_s = mean_std(t_p95)
    g_max_m, g_max_s = mean_std(g_max)
    t_max_m, t_max_s = mean_std(t_max)

    x = np.arange(2)
    w = 0.35
    ax2.bar(x - w/2, [g_p95_m, t_p95_m], w, yerr=[g_p95_s, t_p95_s], capsize=6, label="p95 lead time")
    ax2.bar(x + w/2, [g_max_m, t_max_m], w, yerr=[g_max_s, t_max_s], capsize=6, label="max lead time")
    ax2.set_xticks(x)
    ax2.set_xticklabels(["Greedy", "Tabu"])
    ax2.set_title("Lead time (min)\n(mean ± std)")
    ax2.grid(True, axis="y", linestyle="--", alpha=0.3)
    ax2.legend()

    ax3 = fig2.add_subplot(3, 3, 3)
    g_thr = get("robot_throughput_std", greedy_key)
    t_thr = get("robot_throughput_std", tabu_key)
    g_m, g_s = mean_std(g_thr)
    t_m, t_s = mean_std(t_thr)
    ax3.bar(["Greedy", "Tabu"], [g_m, t_m], yerr=[g_s, t_s], capsize=6)
    ax3.set_title("Robot throughput std\n(mean ± std)")
    ax3.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax4 = fig2.add_subplot(3, 3, 4)
    g = get("avg_delay_min", greedy_key)
    t = get("avg_delay_min", tabu_key)
    g_m, g_s = mean_std(g)
    t_m, t_s = mean_std(t)
    ax4.bar(["Greedy", "Tabu"], [g_m, t_m], yerr=[g_s, t_s], capsize=6)
    ax4.set_title("Avg delay (min)\n(mean ± std)")
    ax4.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax5 = fig2.add_subplot(3, 3, 5)
    g = get("max_delay_min", greedy_key)
    t = get("max_delay_min", tabu_key)
    g_m, g_s = mean_std(g)
    t_m, t_s = mean_std(t)
    ax5.bar(["Greedy", "Tabu"], [g_m, t_m], yerr=[g_s, t_s], capsize=6)
    ax5.set_title("Max delay (min)\n(mean ± std)")
    ax5.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax6 = fig2.add_subplot(3, 3, 6)
    ax6.plot(seeds, get("on_time_rate_pct", greedy_key), marker="o", label="Greedy")
    ax6.plot(seeds, get("on_time_rate_pct", tabu_key), marker="o", label="Tabu")
    ax6.set_title("On-time (%) per seed")
    ax6.set_xlabel("Seed")
    ax6.set_ylabel("On-time (%)")
    ax6.grid(True, linestyle="--", alpha=0.3)
    ax6.legend()

    diff_on_time = [tt - gg for tt, gg in zip(get("on_time_rate_pct", tabu_key), get("on_time_rate_pct", greedy_key))]
    diff_p95_lead = [tt - gg for tt, gg in zip(get("p95_lead_time_min", tabu_key), get("p95_lead_time_min", greedy_key))]
    diff_max_delay = [tt - gg for tt, gg in zip(get("max_delay_min", tabu_key), get("max_delay_min", greedy_key))]

    ax7 = fig2.add_subplot(3, 3, 7)
    labels = ["Δ on-time\n(+) good", "Δ p95 lead\n(−) good", "Δ max delay\n(−) good"]
    vals = [diff_on_time, diff_p95_lead, diff_max_delay]
    means = [mean_std(v)[0] for v in vals]
    stds = [mean_std(v)[1] for v in vals]
    ax7.bar(range(len(labels)), means, yerr=stds, capsize=6)
    ax7.set_xticks(range(len(labels)))
    ax7.set_xticklabels(labels)
    ax7.set_title("Paired difference: Tabu − Greedy\n(mean ± std)")
    ax7.axhline(0.0, linestyle="--", linewidth=1.0, alpha=0.6)
    ax7.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax8 = fig2.add_subplot(3, 3, 8)
    ax8.boxplot(vals, labels=labels, showmeans=True)
    ax8.set_title("Paired difference distribution\n(per seed)")
    ax8.axhline(0.0, linestyle="--", linewidth=1.0, alpha=0.6)
    ax8.grid(True, axis="y", linestyle="--", alpha=0.3)

    ax9 = fig2.add_subplot(3, 3, 9)
    ax9.plot(seeds, diff_on_time, marker="o")
    ax9.set_title("Δ on-time per seed (Tabu − Greedy)")
    ax9.set_xlabel("Seed")
    ax9.set_ylabel("Δ on-time (%)")
    ax9.axhline(0.0, linestyle="--", linewidth=1.0, alpha=0.6)
...
# =============================================================================
# 15) Main runner (figure2 + figure1 + JSON)
# =============================================================================
def main():
    results_by_seed = []
    streams_by_seed = {}

    print("=== Multi-seed experiment (Greedy vs Tabu(best-insertion)) ===")
    ...
    for seed in Config.SEEDS:
        ...
        greedy_summary, greedy_thr = run_simulation_headless(events, "Greedy-only", seed, use_tabu=False)
        tabu_summary, tabu_thr = run_simulation_headless(events, "Tabu(best-insertion)", seed, use_tabu=True)

        results_by_seed.append({
            "seed": seed,
            "greedy_summary": greedy_summary,
            "tabu_summary": tabu_summary,
            "greedy_throughput": greedy_thr,
            "tabu_throughput": tabu_thr,
        })

        print(
            f"- seed={seed} | ontime Greedy={greedy_summary['on_time_rate_pct']}% vs Tabu={tabu_summary['on_time_rate_pct']}% "
            f"| p95Lead {greedy_summary['p95_lead_time_min']} vs {tabu_summary['p95_lead_time_min']} "
            f"| maxDelay {greedy_summary['max_delay_min']} vs {tabu_summary['max_delay_min']}"
        )

    fig2 = plot_comparison_figure2(results_by_seed)

    animate_seed = Config.SEEDS[Config.ANIMATE_SEED_INDEX]
    ...
    # JSON: detailed item records for animation seed (headless re-run)
    ...
    anim_tabu_summary, anim_tabu_thr, anim_item_records = run_tabu_headless_with_records(events_anim, animate_seed)
    ...
    payload = {
        ...
        "animation": {
            ...
            "summary": anim_tabu_summary,
            "throughput": anim_tabu_thr
        }
    }
